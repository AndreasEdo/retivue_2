...
export default function DoctorDashboard() {
  const myCases = mockData.cases.filter((c) => c.doctorId === 1);
    ...
    <div>
        ...
        >
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-[#64748B]">{pendingCount} cases waiting for your review</p>
            </div>
          </div>
        </Link>
        ...
        >
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-[#64748B]">{approvedCount + rejectedCount} cases reviewed</p>
            </div>
          </div>
        </Link>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-[#E2E8F0] p-6">
        <div className="flex justify-between items-center mb-6">
          ...
          <Link to="/doctor/pending" className="text-[#001bca] text-xs font-semibold hover:underline">
            View All
          </Link>
        </div>
        <div className="space-y-4">
          {myCases.slice(0, 5).map((case_) => (
            <div key={case_.id} className="flex items-center justify-between p-4 bg-[#f2f4f6] rounded-lg">
              ...
              <span className="text-xs font-semibold capitalize text-[#64748B]">{case_.status}</span>
            </div>
          ))}
        </div>
      </div>
